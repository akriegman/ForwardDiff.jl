...
This method assumes that `isa(f(x), Union{Number,AbstractArray})`.
"""
@inline function derivative(f::F, x::R) where {F,R<:Number}
    T = typeof(Tag(f, R))
    return extract_derivative(T, f(pluseps(T, x)))
end
...
@inline extract_derivative(::Type{T}, y::Complex{<:Dual}) where {T} = Complex(extract_derivative.(T, reim(y))...)
